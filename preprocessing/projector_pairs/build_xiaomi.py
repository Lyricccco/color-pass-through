"""Build aligned Xiaomi camera-display training pairs from capture triplets."""
...
def main(args=None):
    ...
    parser.add_argument('--output', default="Final_Pair", help="Legacy output name below --dataset")
    parser.add_argument('--output-root', default=None, help="Explicit output directory; preferred for packaged datasets")
    parser.add_argument('--base_exposure', type=float, default=0.216, help="Reference exposure used to normalize captures")
    ...
    parser.add_argument('--raw-roi', type=float, nargs=3, default=(2040, 1526, 1012), metavar=('X', 'Y', 'HEIGHT'))
    parser.add_argument('--isp-roi', type=float, nargs=3, default=(2040, 1142, 1012), metavar=('X', 'Y', 'HEIGHT'))
    ...
    for Filename in tqdm(files):
        ...
        parts = ab.strip().replace(',', ' ').split()
        vals = [float(x) for x in parts]
        AnalogBalance = torch.tensor(vals, dtype=torch.float32)
        var_wb = torch.stack([AnalogBalance[0], AnalogBalance[1], AnalogBalance[2], AnalogBalance[1]]).cuda()
        ...
        mid_sRGB = mid_sRGB * relative_exposure
        
        CamerasRGB = run_pipeline(mid_sRGB, \
                    Camerametadata, 'demosaic', 'gamma').squeeze(0).squeeze(0)
        
        target_wb = torch.tensor([1.0, 1.0, 1.0, 1.0], dtype=torch.float32).cuda()
        eps = 1e-6
        var_wb = torch.clamp(var_wb, min=eps)
        new_wb= target_wb / var_wb
        
        Camerametadata['wb_matrix'] = new_wb.view(1, 1, 1, 4).cuda()
        
        CameraRaw = run_pipeline(Norm_CameraRaw, \
                    Camerametadata, 'normal', 'demosaic')
        
        CameraRaw = CameraRaw * relative_exposure
